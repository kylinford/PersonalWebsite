!function(){
	var bP={};	
	var b=30, bb=170, height=600, buffMargin=1, minHeight=5;
	var c1=[-180, 60], c2=[-30, 250], c3=[10, 300]; //Column positions of labels.
	var colors =["#3366CC", "#DC3912",  "#FF9900","#109618", "#990099", "#0099C6"];
	
	bP.partData = function(data,p1,p2){
		var sData={};
		
		sData.keys=[
			d3.set(data.map(function(d){ return d[p1];})).values(),
			d3.set(data.map(function(d){ return d[p2];})).values()		
		];
		...
		data.forEach(function(d){ 
			sData.data[0][sData.keys[0].indexOf(d[p1])][sData.keys[1].indexOf(d[p2])]+=d[3];
			sData.data[1][sData.keys[1].indexOf(d[p2])][sData.keys[0].indexOf(d[p1])]+=d[3]; 
		});
	...
	function drawPart(data, id, p){
		...
		mainbar.append("rect").attr("class","mainrect")
			.attr("x", 20).attr("y",function(d){ return d.middle-d.height/2; })
		...
		d3.select("#"+id).select(".part"+p).select(".subbars")
			...
			.attr("x", 20).attr("y",function(d){ return d.y})
			.attr("width",b).attr("height",function(d){ return d.h})
			//.style("fill",function(d){ return colors[d.key1%6];});
	}
	...
	function drawEdges(data, id){
		...
		d3.select("#"+id).select(".edges").selectAll(".edge")
			...
			.attr("points", edgePolygon).style("fill",function(d){ return colors[d.key1%6];})
			.style("opacity",0.5).each(function(d) { this._current = d; });	
	}	
	
	function drawHeader(header, id){
		d3.select("#"+id).append("g").attr("class","header").append("text").text(header[2])
			.style("font-size","20").attr("x",128).attr("y",-20).style("text-anchor","middle")
	...
	function edgePolygon(d){
		return [20, d.y1, bb+20, d.y2, bb+20, d.y2+d.h2, 20, d.y1+d.h1].join(" ");
	}	
	...
	bP.draw = function(data, svg){
		data.forEach(function(biP,s){
			svg.append("g")
				...
				.attr("transform", "translate(" + (800 * s) + ",0)");
			...
			[0,1].forEach(function(p){			
				d3.select("#"+biP.id)
					...
					.select(".mainbars")
					.selectAll(".mainbar")
					.on("mouseover", function (d, i) { return bP.selectSegment(data, p, i, s); })
					.on("mouseout",function(d, i){ return bP.deSelectSegment(data, p, i); });	
			});
		});	
	}
	
	bP.selectSegment = function(data, m, s,id){
	    data.forEach(function (k,n) {
